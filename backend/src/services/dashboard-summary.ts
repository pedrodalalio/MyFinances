...
export class DashboardSummaryService {
  ...
  async execute({ userId, month, year }: DashboardSummaryRequest): Promise<DashboardSummaryResponse> {
    // Get current month expenses
    const currentExpenses = await this.expenseRepository.findByMonthAndUser(userId, month, year)
    const currentMonthlyExpenses = currentExpenses.reduce((sum, expense) => sum + Number(expense.amount), 0)

    console.log('Dashboard Debug - Current Expenses:', {
      userId,
      month,
      year,
    ...
    const previousExpenses = await this.expenseRepository.findByMonthAndUser(userId, previousMonth, previousYear)
    const previousMonthlyExpenses = previousExpenses.reduce((sum, expense) => sum + Number(expense.amount), 0)

    // Calculate expenses change
    const expensesChange = previousMonthlyExpenses === 0 ? 0 :
      Number(((currentMonthlyExpenses - previousMonthlyExpenses) / previousMonthlyExpenses) * 100) || 0

    // Get credit card expenses for current month
    const creditCardData = await this.getMonthlyExpensesService.execute({ userId, month, year })
    const totalCreditCardExpenses = creditCardData.total
    const totalInstallments = creditCardData.expenses.length

    console.log('Dashboard Debug - Credit Card:', {
      totalCreditCardExpenses,
      totalInstallments,
      creditCardExpenses: creditCardData.expenses
    })
    ...
    // Get total investments from portfolio
    const portfolio = await this.investmentPortfolioRepository.findByUserId(userId)
    const totalInvestments = portfolio?.assets?.reduce((sum, asset) => sum + Number(asset.current_value || 0), 0) || 0

    // Calculate current balance (salary - expenses - credit card)
    const currentBalance = Number(salary) - Number(currentMonthlyExpenses) - Number(totalCreditCardExpenses)
    ...
    const currentBalanceChange = previousBalance === 0 ? 0 :
      Number(((currentBalance - previousBalance) / Math.abs(previousBalance)) * 100) || 0
    ...
    const savingsRate = salary === 0 ? 0 : ((Number(salary) - Number(currentMonthlyExpenses) - Number(totalCreditCardExpenses)) / Number(salary)) * 100
