...
export class ConfirmMonthService {
  ...
  async execute({
    ...
    const currentMonthData = await this.financialDataRepository.findByUserAndPeriod(userId, month, year)
    ...
    const monthlyInvestments = await this.monthlyInvestmentRepository.findByMonthAndUser(userId, month, year)
    ...
    const installmentsTotal = installments.reduce((sum, inst) => sum + Number(inst.installment_amount), 0)
    ...
    const realExpensesTotal = expenses.reduce((sum, expense) => sum + Number(expense.amount), 0)
    const realInvestmentsTotal = monthlyInvestments.reduce((sum, investment) => sum + Number(investment.amount), 0)
    const realTaxesTotal = taxes.reduce((sum, tax) => sum + Number(tax.amount), 0)
    ...
    const salaryAmount = currentSalary ? Number(currentSalary.amount) : mainIncome
    const totalIncome = salaryAmount + checkingAccount + previousBalance

    // Calcular gastos totais reais
    const totalExpenses = realExpensesTotal + realCreditCardTotal + realTaxesTotal + realInvestmentsTotal
    ...
    console.log('Transferindo para:', nextMonthStr, '/', nextYear)
    console.log('Valor a transferir:', Math.max(0, finalBalance))

    // Verificar se já existe dados para o próximo mês
    ...
    console.log(`Mês ${month}/${year} confirmado. Saldo transferido: ${Math.max(0, finalBalance)}`)
  }
}